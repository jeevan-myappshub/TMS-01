...
const REVIEWER_ID = process.env.NEXT_PUBLIC_REVIEWER || ""; // Default reviewer ID if not set
...
// StatusIcon Component (unchanged)
...
// ReviewLogDialog Component (unchanged)
...
// LogTable Component (updated to include Reviewer column)
function LogTable({
  logs,
  currentPage,
  projects,
        ...
        <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-600">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
            {paginatedLogs.length > 0 ? (
              ...
              <tr>
                <td colSpan={12} className="text-gray-500 dark:text-gray-400 text-center py-6 text-sm">
...
export default function ReviewerDashboard() {
  ...
  const fetchLogs = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const params = new URLSearchParams({ reviewer_id: String(REVIEWER_ID) });
      ...
      // Map logs and capitalize status
      setLogs(
        logsArr.map((log) => ({
          ...log,
      ...
      // Ensure projects is an array and sort by ID
      ...
      if (!projectsArr.length) {
        ...
        // Fallback: Extract unique projects from logs
